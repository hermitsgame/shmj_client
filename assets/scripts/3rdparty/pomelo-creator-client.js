...
(function() {
  ...
  var onclose = function(event) {
      ...
      if (this.need_reconnect === true) {
	    console.log('send need_reconnect');
        pomelo.emit("need_reconnect", event);
      }

      console.error('pomelo socket close: ', event);
    };
  ...
  var connect = function(params, url, cb) {
    ...
    var params = params || {};
    var maxReconnectAttempts = ('maxReconnectAttempts' in params) ? params.maxReconnectAttempts : DEFAULT_MAX_RECONNECT_ATTEMPTS;
    ...
    socket = new WebSocket(url, 'default-protocol');
    socket.binaryType = 'arraybuffer';
    socket.onopen = onopen;
    socket.onmessage = onmessage;
    socket.onerror = onerror;
    socket.onclose = onclose.bind(socket);
    socket.need_reconnect = true;
  };

  pomelo.disconnect = function(heartbeat) {
    if (socket) {
      if (socket.need_reconnect === true) {
		  if (heartbeat)
             pomelo.emit("need_reconnect");

          socket.need_reconnect = false;
      }

      if(socket.disconnect) socket.disconnect();
      if(socket.close) socket.close();

      console.log('client active disconnect!');
      socket = null;
    }
